...
W = 60  # info column width in characters
...
def info_lines(s):
    ...
    return [
        ...
        [],
        rule("Contact"),
        kv("Email", "me@ditrich.dev"),
        kv("LinkedIn", "in/dmitry-marchuk-202944186"),
        [],
        rule("GitHub Stats"),
        kv2(
            "Repos",
...
def render(mode, stats):
    ...
    for i, line in enumerate(ART.strip("\n").split("\n")):
        out.append(
            f'<text x="25" y="{40 + i * 15}" fill="{p["art"]}" xml:space="preserve">{html.escape(line)}</text>'
        )
    for i, segs in enumerate(info_lines(stats)):
        if not segs:
            continue
        spans = "".join(
            f'<tspan fill="{p[c]}">{html.escape(t)}</tspan>' for t, c in segs
        )
        out.append(
            f'<text x="390" y="{45 + i * 21}" xml:space="preserve">{spans}</text>'
        )
    out.append("</svg>")
    return "\n".join(out)
